            ...
            <div className="flex gap-4 overflow-x-auto pb-4 min-h-[400px]">
                {stages.map((stage) => (
                    <KanbanColumn
                        ...
                        isCompleted={stage.id === completedStageId}
                    />
                ))}

                {/* Botão Configurar dentro do Kanban */}
                {onConfigureClick && (
                    <div className="flex-shrink-0 w-[300px] flex flex-col">
                        <button
                            onClick={onConfigureClick}
                            className="flex items-center justify-center gap-2 p-4 h-[56px] bg-muted/50 border-2 border-dashed border-border rounded-xl text-muted-foreground hover:text-primary hover:border-primary/50 hover:bg-primary/5 transition-colors"
                        >
                            ...
                            <span className="font-medium text-sm">Configurar Funil</span>
                        </button>
                    </div>
                )}
            </div>
