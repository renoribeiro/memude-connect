  ...
  try {
    ...
    switch (body.action) {
      ...
      case 'save_settings': {
        ...
        const maxAttempts=Number(input.match_max_attempts??config.match_max_attempts);
        if(!Number.isInteger(maxAttempts)||maxAttempts<1||maxAttempts>20)throw new Error('Máximo de consultas deve ser de 1 a 20');
        ...
        await checked(db.from('visit_automation_config').update({ enabled, match_max_attempts:maxAttempts, intake_enabled: input.intake_enabled === true, closer_phone: closerPhone, group_jid: input.group_jid || '', instance_id: input.instance_id || null, updated_at: new Date().toISOString() }).eq('id', true));
      ...
      case 'match_manual': {
        await checked(db.rpc('visit_match_manual',{p_visit:body.visita_id,p_broker:body.data?.broker_id,p_actor:access.userId}));
        return jsonResponse(req,{success:true});
      }
      case 'history': {
