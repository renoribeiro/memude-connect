...
export function visitPromptText(kind: string, audience: string, reference: string, context: string) {
  ...
  const question = kind === 'assignment' ? 'Você aceita atender esta visita? Responda em até 15 minutos.'
    : kind === 'feedback' ? 'Como foi a visita? Informe o interesse do cliente, objeções e próximo passo. Escreva pelo menos 10 caracteres.'
    : kind === 'attendance' ? 'Houve a visita?'
  ...
  const binary = ['eve', 'h2', 'attendance', 'assignment'].includes(kind);
  const instructions = binary
    ? `Use um botão abaixo ou responda copiando uma das opções:\n${prefix}:sim\n${prefix}:nao`
    : `Responda citando esta mensagem ${kind === 'rating' ? 'com uma nota de 0 a 10' : kind === 'feedback' ? 'com seu feedback' : 'com o motivo'}. Ou copie a referência:\n${prefix}: ${kind === 'rating' ? 'sua nota' : kind === 'feedback' ? 'seu feedback' : 'seu motivo'}`;
...
export const visitEventLabels: Record<string, string> = {
  match_consulting: 'Consultando corretor para a visita', match_accepted: 'Visita confirmada com corretor responsável', match_exhausted: 'Nenhum corretor aceitou: atribuição pelo Closer pendente', match_declined: 'Corretor recusou a consulta', match_timeout: 'Consulta encerrada: próximo corretor', match_unavailable: 'Corretor ficou indisponível: buscando substituto', match_manual: 'Closer indicou corretor para consulta', post_visit_summary: 'Resumo completo da visita', assignment: 'Consulta de aceite enviada',
  scheduled: 'Solicitação de visita recebida', changed: 'Agendamento alterado', missing_broker: 'Visita sem corretor: designar responsável',
  client_confirmed: 'Cliente confirmou presença', broker_confirmed: 'Corretor confirmou presença',
  broker_declined: 'Corretor indisponível: procurando substituto',
  ...
  held: 'Visita realizada: aguardando avaliação e feedback', not_held: 'Visita não realizada: recuperação pendente',
  cancelled: 'Visita cancelada: recuperação pendente', reason: 'Motivo recebido', rating: 'Avaliação do cliente recebida',
  feedback: 'Feedback registrado', withdrawn: 'Desistência registrada pelo Closer', rescheduled: 'Nova visita cadastrada; ciclo reiniciado',
